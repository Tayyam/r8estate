...
import { UnitReview } from '../../../types/project-reviews';

interface UnitReviewsTabProps {
  project: Project;
  unit: Unit;
...
const UnitReviewsTab: React.FC<UnitReviewsTabProps> = ({ 
  project, 
  unit,
  onSuccess, 
  onError 
  ...
  useEffect(() => {
    const loadReviews = async () => {
      try {
        ...
        console.error('Error loading unit reviews:', error);
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'unitReviews'), {
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 mb-2">
              {translations?.unitReviews || 'Unit Reviews'}
            </h2>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-gray-50 rounded-xl">
          ...
          <p className="text-gray-500 max-w-md mx-auto">
            {translations?.beTheFirstToReview || 'Be the first to review this unit and share your experience with others.'}
          </p>
        </div>
      )}
    </div>
