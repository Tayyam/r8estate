// ترجمات صفحة البداية والهيرو
export const arHero = {
  heroTitle: 'منصة تقييم القطاع العقاري الأولى في مصر',
  heroSubtitle: 'اكتشف وقيم أفضل العقارات والمطورين في مصر. تقييمات حقيقية من عملاء حقيقيين',
  searchPlaceholder: 'ابحث عن شركة...',
  allCategories: 'جميع الفئات',
  allLocations: 'جميع المواقع',
  shareExperiencePrompt: 'لسه طالع من تجربة مع شركة؟',
  shareWithUs: 'شاركها معانا',
  search: 'بحث',
  explore: 'استكشف',
  
  // Top rated companies
  topRatedCompanies: 'الشركات الأعلى تقييماً',
  
  // Latest reviews
  latestReviews: 'أحدث التقييمات',
  noReviewsAvailable: 'لا توجد تقييمات متاحة حالياً.',
  reviews: 'تقييمات',
  
  // Categories
  browseByCategory: 'تصفح حسب الفئة',
  developers: 'المطورين العقاريين',
  brokers: 'الوسطاء العقاريين',
  consultants: 'المستشارين العقاريين',
  noCategoriesFound: 'لم يتم العثور على فئات'
};